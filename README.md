...
笔者试用了下，在Pi2上编译出错，原因是结构出的初始化问题，Pi2的g++版本似乎不支持结构体的复合字面量初始化方式。
因此笔者修改了初始化的代码，试着编译了用于MPU6050的测试代码，通过了，也确实可以争取拿到roll、pitch、yaw的数据。
不过如果选择编译用于MPU9250的版本，却不能完成MPU的初始化，如法正确工作。这个后面有空再修改，我也在原作者的github上提了issue。
